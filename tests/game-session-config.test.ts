...
describe('mode to game session integration', () => {
  it('maps a valid Free Battle config into the shared GameSessionConfig', () => {
    ...
    const freeBattle = createFreeBattleSessionConfig(profile, {
      playerHeroId: 'vanguard', cpuHeroId: 'arcanist', cpuDifficulty: 'normal',
    });
    expect(freeBattle).not.toBe('hero-locked');
    if (freeBattle === 'hero-locked') return;
    expect(fromFreeBattleConfig(freeBattle)).toEqual({
      mode: { kind: 'free-battle' }, playerHeroId: 'vanguard', cpuHeroId: 'arcanist', cpuDifficulty: 'normal',
    });
  });

  it('creates the first Story encounter config from a new profile', () => {
    expect(createStoryGameSessionConfig(createPlayerProfile(), 'E1-1')).toEqual({
      mode: { kind: 'story', encounterId: 'E1-1' }, playerHeroId: 'vanguard', cpuHeroId: 'vanguard', cpuDifficulty: 'easy',
    });
  });
  ...
  it('rejects locked Story encounters until progression unlocks them', () => {
    ...
    expect(createStoryGameSessionConfig(profile, 'E1-2')).toMatchObject({ mode: { kind: 'story', encounterId: 'E1-2' }, cpuDifficulty: 'easy' });
  });

  it('bootstraps Story and Free Battle through the same fixed-turn session state contract', () => {
    ...
    expect(session.state).not.toHaveProperty('timing');
  });

  it('preserves the Story boss Normal difficulty through session bootstrap', () => {
    let profile = createPlayerProfile();
    for (const id of ['E1-1', 'E1-2', 'E1-3', 'E1-4', 'E1-5'] as const) profile = completeStoryEncounter(profile, id);
