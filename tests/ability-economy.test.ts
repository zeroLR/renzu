...
describe('hero ability economy', () => {
  ...
  it('supports condition-gated resources as a flow-neutral economy primitive', () => {
    const initial = setAbilityResource(createAbilityStates(), 1, 'pressure', 2);
    const rule = { kind: 'resource-and-condition', resourceId: 'pressure', amount: 2, conditionId: 'contested-space' } as const;
  ...
  it('clamps active resources at zero', () => {
    const state = setAbilityResource(createAbilityStates(), 1, 'mana', -5);
    expect(getAbilityResource(state, 1, 'mana')).toBe(0);
  });
});
